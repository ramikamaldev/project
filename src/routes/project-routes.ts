...
import {retrieve_encryption_key, retrieve_encrypted_documents} from "../dao";
import {store_in_database, encrypt_project_data, extract_data_with_matching_encryption_key_and_decrypt} from "../project-functionality/project-data-func";
...
async function store_data(req: express.Request, res: express.Response) {
  try {
    ...
    const unencrypted_json = req.body.value;
    const encryption_key = req.body.encryption_key;
    const encrypted_json = encrypt_project_data(unencrypted_json, encryption_key);
...
async function retrieve_data(req: express.Request, res: express.Response) {
  try {
    let id = req.body.id;
    const encryption_key = req.body.decryption_key;
    id = id.replace("*", ".");
    const regex_query = new RegExp(`(${id})\\w`);
    const encrypted_jsons_array: any = await retrieve_encrypted_documents(regex_query);
    const stored_encryption_keys = await retrieve_encryption_key(regex_query);
    const decrypted_json_array = extract_data_with_matching_encryption_key_and_decrypt(encryption_key, stored_encryption_keys, encrypted_jsons_array);
