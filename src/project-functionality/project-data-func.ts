...
export function decrypt_project_data(encrypted_json_param: any, encryption_key: string) {
  ...
  for (const key in encrypted_json_param["value"]) {
    if (encrypted_json_param["value"].hasOwnProperty(key)) {
      ...
      const decrypted_element = AES.decrypt(encrypted_element as string, encryption_key).toString();
...
export function encrypt_project_data(unencrypted_json: Object, encryption_key: string) {
  ...
  for (const key in unencrypted_json) {
    if (unencrypted_json.hasOwnProperty(key)) {
      ...
      const encrypted_element = AES.encrypt(unencrypted_element.toString(), encryption_key).toString();
...
export function retrieve_from_database(id) {
  return new Promise(async function(resolve, reject) {
    try {
      const encryption_key = await retrieve_encryption_key(id);
      const encrypted_json = await retrieve_encrypted_documents(id);
    ...
    encryption_key_loop: for (let j = 0; j < stored_encryption_keys.length; j++) {
      if (encrypted_jsons_array[i]["document_id"] === stored_encryption_keys[j]["document_id"]) {
        if (encryption_key === stored_encryption_keys[j]["encryption_key"]) {
          const decrypted_json = decrypt_project_data(encrypted_jsons_array[i], encryption_key);
