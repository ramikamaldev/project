import {project_data_model} from "../mongo-schemas/project-data-coll-schema";
import {check_if_collection_exists_and_execute} from "../../../common-functions/utility-functions";


/**
 * create_or_update_project_entry - This function utilises the create and findOneAndUpdate functionality of MongoDB.
 ...
 */
export function create_or_update_project_entry(id, encrypted_json) {
  const create_document_if_collection_does_not_exist = function(resolve, reject, encrypted_json) {
    project_data_model.create({document_id: id, value: encrypted_json}).then(
        function(response) {
  ...
  const update_document_if_collection_exists = function(resolve, reject, encrypted_json) {
    project_data_model.findOneAndUpdate({document_id: id}, {$set: {value: encrypted_json}}, {upsert: true}).then(
        function(response) {
          return resolve(response);
        }).catch(
        function(err) {
  ...
  return new Promise(function(resolve, reject) {
    check_if_collection_exists_and_execute(resolve, reject, project_data_model, update_document_if_collection_exists, encrypted_json, create_document_if_collection_does_not_exist);
  });
}
...
export function retrieve_encrypted_documents(regex_query) {
  return new Promise(async function(resolve, reject) {
    try {
      const encrypted_documents = await project_data_model.find({
        document_id:
                {
                  $regex: regex_query,
                },
