import {project_encryption_key_model} from "../mongo-schemas/project-encryption-key-coll-schema";
...
export async function create_or_update_encryption_key(id, encryption_key) {
  const create_encryption_key_document = function(resolve, reject, encryption_key) {
    project_encryption_key_model.create({document_id: id, encryption_key}).then(
        function(response) {
  ...
  const update_encryption_key_document = function(resolve, reject, encryption_key) {
    project_encryption_key_model.findOneAndUpdate({document_id: id}, {$set: {encryption_key}}, {upsert: true}).then(
        function(response) {
          return resolve(response);
        }).catch(
        function(err) {
  ...
  return new Promise(function(resolve, reject) {
    check_if_collection_exists_and_execute(resolve, reject, project_encryption_key_model, update_encryption_key_document, encryption_key, create_encryption_key_document);
  });
}
...
export function retrieve_encryption_key(id) {
  return new Promise(async function(resolve, reject) {
    try {
      const encryption_key = await project_encryption_key_model.find({document_id: id}).exec();
